    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/employee/staff/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/employee/employee/infoById`),
              method: 'post',
              data: this.$http.adornData({
                'epId': this.dataForm.id || undefined,
                'epMessage': this.dataForm.message,
              })
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.$message({
