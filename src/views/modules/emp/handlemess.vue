<template>
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        <el-button @click="getDataList()">搜索邮件</el-button>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-tag v-if="scope.row.sendstatus == 1">未处理</el-tag>
      ...
      <el-table-column
        ...
        label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">查看邮件</el-button>
          <el-button type="text" size="small" @click="handle(scope.row.id)">处理完成</el-button> 
          <!-- <el-button type="text" size="small" @click="UpdateHandle(scope.row.id)">查看邮件</el-button> -->
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
  import AddOrUpdate from './handlemess-add-or-update'
  import AddOrUpdate1 from './backmess-add-or-update'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      handle(id){
        this.addOrUpdateVisible1=true,
         this.$http({
            url: this.$http.adornUrl(`/employee/sendmess/updateByHandle2/${id}`),
            method: 'post',
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/employee/sendmess/list12'),
          method: 'get',
      ...
      addOrUpdateHandle (id) {
        ...
        this.$http({
            url: this.$http.adornUrl(`/employee/sendmess/updateByHandle1/${id}`),
            method: 'post',
            //data: this.$http.adornData(ids, false)
          }).then(({data}) => {
            if (data && data.code === 0) {
