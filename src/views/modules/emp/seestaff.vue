<template>
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getempList()">
      ...
      <el-form-item>
        <el-button @click="getempList()">查询申诉信息及结果</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
      ...
      <el-table-column
        ...
        label="申诉状态">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.epEmpoyeeStatus == 1">已提交</el-tag>
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button type="text" size="small" @click="deleteHandle(scope.row.epId)">撤销申诉</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
  import AddOrUpdate from './updatestaff'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
        ...
        this.$http({
          url: this.$http.adornUrl('/employee/staff/listemp'),
          method: 'get',
      ...
      deleteHandle (id) {
        this.$confirm(`确定删除申请信息?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/employee/staff/delete'),
            method: 'post',
